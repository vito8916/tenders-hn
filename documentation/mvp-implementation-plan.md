...
| O6 | Central sync cadence | Start every **3 hours** with a 7-day overlap window (spec §6); adjust after Phase 1 measurements. | Phase 1 |
...
- `docs.download` job: download each new link, hash, store under `source-documents/<process_id>/<sha256>.pdf`, create `document_versions` once per hash.
- `docs.extract` job: `pdftotext` per page; pages with too little text go to Tesseract `spa`; the `vision` role is used only if enabled (O2); write `document_pages` with method and confidence; set `extraction_status` (`partial` when some pages fail — shown as partial analysis).
